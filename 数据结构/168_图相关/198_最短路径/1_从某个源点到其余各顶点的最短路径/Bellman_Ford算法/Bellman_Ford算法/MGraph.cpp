...
bool CreateGraph(MGraph &G)			//选择创建				//汇总
{
	printf("Which kind of Graph do you want to create?\n");
	printf("0---DG  1---DN  \n2---UDG  3---UDN\n");
	scanf("%d",&G.kind);
	switch(G.kind)				//根据图的类型选择函数的使用
	{
	case DG:
		return CreateGraph_DG(G);
	case DN:
		return CreateGraph_DN(G);
	case UDG:
		return CreateGraph_UDG(G);
	case UDN:
		return CreateGraph_UDN(G);
	default:
		return false;
	}
}

//每种图的创建方式都差不多  主要区别在于有向无向 边是否有权
bool CreateGraph_DG(MGraph &G)							//构造有向图G
{
	FILE *fp;
	VexType v1,v2;
	printf("Please input the name the file : ");
	char filename[20];
	scanf("%s",filename);
	...
	for(i=0;i<G.vexnum;i++)											//读入顶点数
		fscanf(fp,"%s",G.vexs[i]);
	for(i=0;i<G.vexnum;i++)							//初始化整个邻接矩阵
...
bool CreateGraph_DN(MGraph &G,Edge *&edge)							//有向网
{
	FILE *p;
	char filename[20];
	...
	edge = (Edge*)malloc(sizeof(Edge)*G.arcnum);
	...
	for(k=0;k<G.arcnum;k++)
	{
		...
		edge[k].u = i = LocateVex(G,v1);
		edge[k].v = j = LocateVex(G,v2);
		edge[k].w = G.arcs[i][j].adj = w;
	}
	return true;
}	
bool CreateGraph_UDG(MGraph &G)								//无向图
{
	int i,j;
	VexType v1,v2;
